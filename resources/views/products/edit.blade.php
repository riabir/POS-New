<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Product: ') }} <span class="text-indigo-600">{{ $product->model }}</span>
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="post" action="{{ route('products.update', $product->id) }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                             ...
                             <div>
                                ...
                                <input type="text" id="model" name="model" value="{{ old('model', $product->model) }}" class="mt-1 block w-full rounded-md shadow-sm" required>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="header" name="header" rows="3" class="mt-1 block w-full rounded-md shadow-sm" required>{{ old('header', $product->header) }}</textarea>
                        </div>
                        
                        {{-- MRP --}}
                        <div>
                            <label for="mrp" class="block font-medium text-sm">MRP (Selling Price)</label>
                            <input type="number" step="0.01" id="mrp" name="mrp" value="{{ old('mrp', $product->mrp) }}" class="mt-1 block w-full rounded-md shadow-sm" required>
                        </div>
                        
                        {{-- Description --}}
                        <div>
                            <label for="description" class="block font-medium text-sm">Description (Marketing Pitch)</label>
                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full rounded-md shadow-sm">{{ old('description', $product->description) }}</textarea>
                        </div>
                        ...
                        <div class="border-t pt-6 mt-6">
                            ...
                            <div id="specifications-container" class="mt-2 space-y-3">
                                {{-- JS will populate this with existing specs --}}
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Dependent Dropdown Logic (same as create)
            ...
            // Pre-populate existing specifications from the product data
            ...
            if (Object.keys(existingSpecs).length > 0) {
                for (const [key, value] of Object.entries(existingSpecs)) {
                 ...
                 // Add one empty row if there are no existing specs
                addSpecRow();
            }
        });
    </script>
