<x-app-layout>
    ...
    {{-- The styles for the badges can be moved to your main app.css file, 
         but keeping them here is also fine for a single-use case. --}}
    <style>
                        ...
                        <table class="styled-table">
                            ...
                            <tbody>
                                @forelse($sales as $sale)
                                    {{-- MODIFICATION: Add class and data-attribute to the <tr> --}}
                                    <tr class="clickable-row" data-sale-id="{{ $sale->id }}" title="Click to view invoice preview">
                                        ...
                                        {{-- Remarks column removed for brevity, you can add it back if needed --}}
                                        ...
                                        <td>
                                            <div class="actions-container">                                              
                                                <form method="POST" action="{{ route('sales.destroy', $sale->id) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    {{-- MODIFICATION: Stop click from bubbling up to the row from the button --}}
                                                    <button onclick="event.stopPropagation(); return confirm('Are you sure?')" type="submit" class="btn btn-sm btn-danger">Delete</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div id="invoicePreviewContent" class="mt-4">
    ...
    {{-- =================================================================== --}}
    @push('scripts') {{-- If you have a @stack('scripts') in your layout --}}
    <script>
            ...
            rows.forEach(row => {
                row.addEventListener('click', function () {
                    ...
                    fetch(url)
                        ...
                        .catch(error => {
                            invoicePreviewContent.innerHTML = `<div class="text-center py-10 text-red-500">Failed to load invoice. Please try again.</div>`;
                            console.error('Error fetching invoice:', error);
                        });
                });
            });

            // Prevent clicks on action buttons from triggering the row click
